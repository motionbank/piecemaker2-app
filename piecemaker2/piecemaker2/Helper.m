...
+ (void)createDatabaseIfNotExist:(NSString*) database {
    ...
    if([[result valueForKey:@"code"] intValue] == 0 && [[result valueForKey:@"result"] length] > 0) {
        ...
        NSDictionary *result = [Helper runCommand:[NSString stringWithFormat:@"createdb --encoding UTF8 --port=50725 --host=localhost piecemaker2_%@", database] waitUntilExit:TRUE];
